  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Replace with your actual user profile API endpoint
        const response = await fetch("https://happywedz.com/api/user/" + userInfo.id, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const budgetRes = await fetch( // This should be user-specific
        ...
        const guestsRes = await fetch(`https://happywedz.com/api/guestlist/${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        // Fetch Task Summary
        const tasksRes = await fetch(`https://happywedz.com/api/checklist_new/${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const tasksData = await tasksRes.json();
        if (tasksData.success) {
          const completedTasks = tasksData.data.filter(t => t.status === 'completed').length;
          setTaskStats({ total: tasksData.data.length, completed: completedTasks });
        }
      } catch (error) {
              ...
              <div className="card rounded-0 border-0 shadow-sm">
                <div className="row text-center g-0">
                  ...
                  <div className="col py-3 border-end">
                    ...
                    <small className="text-muted">{taskStats.completed} of {taskStats.total || 20}</small>
                  </div>
                  <div className="col py-3">
                    <h6 className="mb-1 fw-semibold text-dark">
                      Guest Attending
                    </h6>
                    <small className="text-muted">{guestStats.attending} of {guestStats.total || 100}</small>
                  </div>
                </div>
              </div>
            </div>
