...
const WishlistPage = () => {
  ...
  const updateWishlistField = async (id, field, value) => {
    try {
      ...
      if (data.success)
        setWishlist(
          wishlist.map((item) => (item.id === id ? { ...item, [field]: value } : item))
        );
    } catch (err) {
      console.error(err);
    }
  ...
  const updateRating = (id, newRating) => updateWishlistField(id, "rating", newRating);
  const updateStatus = (id, status) => updateWishlistField(id, "status", status);
  const updatePricing = (id, pricing) => updateWishlistField(id, "pricing", pricing);
